...
> [!note] ESKF 并未解决根本问题
> [[误差状态卡尔曼滤波]] 用误差状态在线性化中解耦了四元数范数约束，但它的误差传播矩阵仍然依赖于名义状态的实时估计——名义状态偏离真实值时，Jacobian 同样失真。区别在于：ESKF 的误差状态是 3 维（无冗余），数值稳定性优于全状态 EKF，但收敛域与真实轨迹解耦的性质只有 IEKF 能做到。
...
- [[误差状态卡尔曼滤波]] — ESKF 与 IEKF 的对比：两种不同的从流形解耦线性化的策略
- [[AVNet 不变扩展卡尔曼姿态]] — InEKF + 深度学习的工程应用实例
- [[Matrix Fisher SO3概率姿态]] — 另一种处理 SO(3) 不确定性的框架，直接对旋转矩阵概率建模
- [[IMU姿态解算算法演进]] — 算法全景中 IEKF/InEKF 的位置
- [[2026-06-24 - Solà Error-State Kalman Filter]] — Error-State KF 原文，与 IEKF 对照理解
