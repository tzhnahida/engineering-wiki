...
> **DBI (Display Bus Interface)** 是 MIPI 联盟定义的并行显示总线接口，基于经典的微处理器总线协议（Intel 8080 和 Motorola 6800），用于主机通过并行数据总线控制显示模组。DBI 通常与 [DCS](MIPI DCS.md) 命令集配合使用。
...
DBI 不定义命令语义——它只提供传输管道。显示控制命令由 [DCS](MIPI DCS.md) 定义：
...
- [视频显示/MIPI DSI](MIPI DSI.md) — DSI（DBI 的串行替代方案）
- [视频显示/MIPI DCS](MIPI DCS.md) — Display Command Set（DBI 上运行的命令层）
- [视频显示/MIPI DPI](MIPI DPI.md) — 并行像素接口（无 GRAM，实时刷新）
