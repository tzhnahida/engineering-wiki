...
- 所有 TMDS 输出端必须具有短路保护
- +5V 引脚必须有过流保护
- ESD 保护器件（如 [[USBLC6-2SC6]]）应放置在连接器近端，钳位电压需匹配 3.3 V TMDS 共模
...
- **ESD 保护**：[[USBLC6-2SC6]] 或同类 TVS 二极管，每对 TMDS 一个，布局在连接器 5 mm 以内
...
- [[HDMI Type A]] — Type A 连接器实体页（含封装信息）
- [[HDMI Type D]] — Type D Micro 连接器实体页
- [[TC358870]] — HDMI 1.4 到 MIPI DSI 桥接芯片
