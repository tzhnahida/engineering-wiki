...
- **四开关 Buck-Boost** 拓扑 → [[DC-DC降压转换器]]（buck 原理的延伸：buck、boost 两桥臂共用一颗电感）
...
- **PFM/FPWM 双模式** → [[轻载效率模式]]
- **展频 ±7% + 外部时钟同步** → [[展频谱与EMI抑制]]
- **保护**：可编程 UVLO、平均+峰值双重限流、输出恒流环、OVP、短路打嗝、热关断 → [[电源保护机制]]
...
（49.9kΩ + 0.01µF → 约 716Hz，推算。）DITH/SYNC 接地或拉高 >1.2V 即禁用展频；使用外同步时展频自动禁用 → [[展频谱与EMI抑制]]
...
内部等效检流电阻 RSENSE = 0.055Ω，误差放大器跨导 GEA = 190µA/V；CP < 10pF 时可省略。目标相位裕度 >45°、增益裕度 >10dB。原理详见 [[环路补偿]]。
...
---
- [[DC-DC降压转换器]] · [[展频谱与EMI抑制]] · [[轻载效率模式]] · [[电源保护机制]] · [[环路补偿]]
