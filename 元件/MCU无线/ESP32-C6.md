...
- **硬件 TSF**: 自动 Beacon 监测由硬件 TSF 定时器完成，CPU 可睡——同一定时器也是 [通讯网络/TSF WiFi 时间同步](../../知识/通讯网络/TSF WiFi 时间同步.md) 的 µs 级同步基准。
...
- [通讯网络/TSF WiFi 时间同步](../../知识/通讯网络/TSF WiFi 时间同步.md) — 基于 C6/S3 硬件 TSF 定时器的 µs 级多节点同步方案
